...
void Graphique::print() {
    ...
    for (int x = 0; x < grille.getGridHeight(); ++x) {
        for (int y = 0; y < grille.getGridWidth(); ++y) {
            ...
            sf::RectangleShape cellule(sf::Vector2f(grille.getCellSize(), grille.getCellSize()));
            if (grille.getGrid()[x][y].getIsAlive())
            {
                cellule.setPosition(y * grille.getCellSize(), x * grille.getCellSize());
                window.draw(cellule);
            }
        }
    }
...
{   
    ...
    setupButton(boutonFin, textFin, "L'automate n'évolue plus !", font, 1.0f, 1.0f, 0.01f, 0.01f, window);
    //end
